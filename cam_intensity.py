...
#%% plotting
        ...
        if showmovie or writemovie:
            ...
            ax.imshow(mim,cmap='bwr',alpha=.15,vmin=0,vmax=1,origin='lower') #radar beam
...
def update(imgs,mask,t,hi,ht,showmovie,writemovie,writer):
    Nimg = imgs.shape[0]
    bmean =  empty(Nimg); bmin = empty(Nimg); bmax = empty(Nimg); bvar = empty(Nimg)
    ...
    for i,img in enumerate(imgs):
        ...
        if showmovie or writemovie:
            ...
            draw(),pause(0.001)
            if writemovie: writer.grab_frame(facecolor='k')
...
if __name__ == '__main__':
    ...
    p = ArgumentParser()
    p.add_argument('-s','--showmovie', help="show live movie (takes a while, don't use if you want quick summary plot",action='store_true')
    p.add_argument('-w','--writemovie',help="write a LOSSLESS movie file that is viewable on phones, etc. easily from the HDF5 file",action='store_true')
    p = p.parse_args()
