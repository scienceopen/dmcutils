...
# %% plotting
        ...
        if showmovie or writemovie:
            ...
            hi = ax.imshow(imgs[0, ...], cmap='gray', norm=LogNorm(), origin='lower',
                           vmin=vlim[0], vmax=vlim[1])  # primes display
            fg.colorbar(hi)

            mim = masked_where(~mask, ones(imgs.shape[1:]))
            ax.imshow(mim, cmap='bwr', alpha=.15, vmin=0,
                      vmax=1, origin='lower')  # radar beam
            ax.set_xlabel('x-pixel')
            ax.set_ylabel('y-pixel')

            ax.scatter(boresight_rc[1], boresight_rc[0],
                       s=150, marker='*', alpha=0.3, color='b')
        ...
        if showmovie or writemovie:
            ...
            bmean, bmin, bmax, bvar = update(
                imgs, mask, t, None, None, showmovie, writemovie, None)
...
if __name__ == '__main__':
    ...
    p = ArgumentParser()
    p.add_argument('-s', '--showmovie',
                   help="show live movie (takes a while, don't use if you want quick summary plot", action='store_true')
    p.add_argument('-w', '--writemovie',
                   help="write a LOSSLESS movie file that is viewable on phones, etc. easily from the HDF5 file", action='store_true')
    P = p.parse_args()
